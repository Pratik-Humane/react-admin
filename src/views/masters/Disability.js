...
const { getDisabilities } = MasterService();
class Disability extends Component {
  state = { resultdata: [], isLoading: false, offset: 0, limit: 10, count: 0, pages: [], selected: 0, };

  componentDidMount() {
    this.getDisabilityListData(this.state.offset, this.state.limit, this.state.search);
  }

  // get Disability list
  getDisabilityListData = async (offset, limit, search) => {
    this.setState({ isLoading: false });
    getDisabilities(offset, limit, search).then(async (res) => {
  ...
  changePage = (offset) => {
    ...
    this.getDisabilityListData(offset * this.state.limit, this.state.limit);
  };

  // search filter
  searchFilter = () => {
    this.getDisabilityListData(0, 10, this.state.search);
  };

  // reset filter
  resetFilter = () => {
    this.setState({ ...this.state, search: null });
    this.getDisabilityListData(this.state.selected, this.state.limit, null);
    ...
    return (
      <>
          ...
          <div className="card">
            <div className="card-body">
              <div className="col-lg-10">
                ...
                &nbsp;&nbsp;
                <button type="button" className="btn btn-danger" onClick={this.resetFilter} > Reset
                </button>
              </div>
            </div>
          </div>
        ...
        <div className="col-lg-12 grid-margin stretch-card">
          <div className="card">
            <div className="card-body">
              <h4 className="card-title">Manage Disabilities</h4>
