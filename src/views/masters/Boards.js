...
const { getBoards } = MasterService();
class Boards extends Component {
  state = { boards: [], isLoading: false, offset: 0, limit: 10, count: 0, pages: [], selected: 0, };

  componentDidMount() {
    this.getBoardsListData(this.state.offset, this.state.limit, this.state.search);
  }

  // get Boards list
  getBoardsListData = async (offset, limit, search) => {
    getBoards(offset, limit, search).then(async (res) => {
      ...
      if (response.data.status === 200) {
        ...
        this.setState({ isLoading: true, boards: response.data.result, count: response.data.count, pages: pages, });
      } else {
  ...
  changePage = (offset) => {
    ...
    this.getBoardsListData(offset * this.state.limit, this.state.limit);
  };

  // search filter
  searchFilter = () => {
    this.getBoardsListData(0, 10, this.state.search);
  };

  // reset filter
  resetFilter = () => {
    this.setState({ ...this.state, search: null });
    this.getBoardsListData(this.state.selected, this.state.limit, null);
    ...
    return (
      <>
        ...
        <div className="col-lg-12 grid-margin stretch-card">
          <div className="card">
            <div className="card-body">
              <h4 className="card-title">Manage Boards</h4>
                    ...
                    {this.state.boards.map((board, index) => {
                      return (
                        <tr key={index}>
                          ...
                          <td>{board.name}</td>
                          <td> {board.status === "1" ? ( <span className="text text-success">Active</span> ) : ( <span className="text text-danger">Inactive</span> )} </td>
                        </tr>
                      );
                    })}
                  </tbody>
