...
const { getMotherTongue } = MasterService1();
class MotherTongue extends Component {
  state = { result: [], isLoading: false, offset: 0, limit: 10, count: 0, pages: [], selected: 0, };

  componentDidMount() {
    this.getMotherTongueData(this.state.offset, this.state.limit, this.state.search);
  }

  // get Mother Tongue list
  getMotherTongueData = async (offset, limit, search) => {
    getMotherTongue(offset, limit, search).then(async (res) => {
  ...
  changePage = (offset) => {
    ...
    this.getMotherTongueData(offset * this.state.limit, this.state.limit);
  };

  // search filter
  searchFilter = () => {
    this.getMotherTongueData(0, 10, this.state.search);
  };

  // reset filter
  resetFilter = () => {
    this.setState({ ...this.state, search: null });
    this.getMotherTongueData(this.state.selected, this.state.limit, null);
    ...
    return (
      <>
        ...
        <div className="col-lg-12 grid-margin stretch-card">
          <div className="card">
            <div className="card-body">
              <h4 className="card-title">Manage Mother Tongue</h4>
