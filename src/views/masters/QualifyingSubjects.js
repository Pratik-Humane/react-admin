...
const { getQualifiedSubject } = MasterService1();
export default class QualifyingSubjects extends Component {
  state = { result: [], isLoading: false, offset: 0, limit: 10, count: 0, pages: [], selected: 0, search: null, };

  componentDidMount() {
    this.getQualifiedSubListData( this.state.offset, this.state.limit, this.state.search );
  }

  // get subcategory list
  getQualifiedSubListData = async (offset, limit,search) => {
    this.setState({ isLoading: false });
      getQualifiedSubject(offset, limit, search).then(async (res) => {
  ...
  changePage = (offset) => {
    ...
    this.getQualifiedSubListData( offset * this.state.limit, this.state.limit, this.state.search );
  };

  // search filter
  searchFilter = () => {
      this.getQualifiedSubListData(0, 10, this.state.search);
  }

  // reset filter
  resetFilter = () => {
      this.setState({...this.state,search:null});
      this.getQualifiedSubListData( this.state.selected, this.state.limit, null );
    ...
    return (
      <>
        ...
        <div className="col-lg-12 grid-margin stretch-card">
          <div className="card">
            <div className="card-body">
              <h4 className="card-title">Manage Qualifying Test Subject</h4>
                    ...
                    {this.state.result.map((row, index) => {
                      return (
                        <tr key={index}>
                          ...
                          <td>{row.entrance && row.entrance.exam_name}</td>
                          <td>{row.subject}</td>
