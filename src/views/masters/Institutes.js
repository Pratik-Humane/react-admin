...
const { getInstitutes } = MasterService();
class Institutes extends Component {
  state = { institutes: [], isLoading: false, offset: 0, limit: 10, count: 0, pages: [], selected: 0, };

  componentDidMount() {
    this.getInstitutesListData( this.state.offset, this.state.limit, this.state.search );
  }

  // get Institutes list
  getInstitutesListData = async (offset, limit, search) => {
    this.setState({ isLoading: false });
    getInstitutes(offset, limit, search).then(async (res) => {
      ...
      if (response.data.status === 200) {
        ...
        this.setState({ isLoading: true, institutes: response.data.result, count: response.data.count, pages: pages, });
      } else {
  ...
  changePage = (offset) => {
    ...
    this.getInstitutesListData(offset * this.state.limit, this.state.limit);
  };

  // search filter
  searchFilter = () => {
    this.getInstitutesListData(0, 10, this.state.search);
  };

  // reset filter
  resetFilter = () => {
    this.setState({ ...this.state, search: null });
    this.getInstitutesListData(this.state.selected, this.state.limit, null);
    ...
    return (
      <>
        ...
        <div className="col-lg-12 grid-margin stretch-card">
          <div className="card">
            <div className="card-body">
              <h4 className="card-title">Manage Institutes</h4>
                    ...
                    {this.state.institutes.map((institute, index) => {
                      return (
                        <tr key={index}>
                          ...
                          <td> {institute.status === "1" ? ( <span className="text text-success">Active</span> ) : ( <span className="text text-danger">Inactive</span> )} </td>
                        </tr>
                      );
                    })}
                  </tbody>
