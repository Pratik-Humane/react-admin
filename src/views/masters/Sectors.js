...
const { getSectors } = MasterService();
class Sectors extends Component {
  state = { sectors: [], isLoading: false, offset: 0, limit: 10, count: 0, pages: [], selected: 0, };

  componentDidMount() {
    this.getSectorsListData(this.state.offset, this.state.limit, this.state.search);
  }

  // get sectors list
  getSectorsListData = async (offset, limit, search) => {
    this.setState({ isLoading: false });
    getSectors(offset, limit, search).then(async (res) => {
      ...
      if (response.data.status === 200) {
        ...
        this.setState({ isLoading: true, sectors: response.data.result, count: response.data.count, pages: pages, });
      } else {
  ...
  changePage = (offset) => {
    ...
    this.getSectorsListData(offset * this.state.limit, this.state.limit);
  };

  // search filter
  searchFilter = () => {
    this.getSectorsListData(0, 10, this.state.search);
  };

  // reset filter
  resetFilter = () => {
    this.setState({ ...this.state, search: null });
    this.getSectorsListData(this.state.selected, this.state.limit, null);
    ...
    return (
      <>
        ...
        <div className="col-lg-12 grid-margin stretch-card">
          <div className="card">
            <div className="card-body">
              <h4 className="card-title">Manage Sectors</h4>
                    ...
                    {this.state.sectors.map((sector, index) => {
                      return (
                        <tr key={index}>
                          ...
                          <td> {sector.status === "1" ? (<span className="text text-success">Active</span>) : (<span className="text text-danger">Inactive</span>)} </td>
                        </tr>
                      );
                    })}
                  </tbody>
