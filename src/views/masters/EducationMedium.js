...
const { getEducationMedium } = MasterService();
class EducationMedium extends Component {
  state = { resultdata: [], isLoading: false, offset: 0, limit: 10, count: 0, pages: [], selected: 0, };

  componentDidMount() {
    this.getEducationMediumListData(this.state.offset, this.state.limit, this.state.search);
  }

  // get EducationMedium list
  getEducationMediumListData = async (offset, limit, search) => {
    this.setState({ isLoading: false });
    getEducationMedium(offset, limit, search).then(async (res) => {
      ...
      if (response.data.status === 200) {
        ...
        this.setState({ isLoading: true, resultdata: response.data.result, count: response.data.count, pages: pages, });
      } else {
  ...
  changePage = (offset) => {
    ...
    this.getEducationMediumListData(offset * this.state.limit, this.state.limit);
  };

  // search filter
  searchFilter = () => {
    this.getEducationMediumListData(0, 10, this.state.search);
  };

  // reset filter
  resetFilter = () => {
    this.setState({ ...this.state, search: null });
    this.getEducationMediumListData(this.state.selected, this.state.limit, null);
    ...
    return (
      <>
        ...
        <div className="col-lg-12 grid-margin stretch-card">
          <div className="card">
            <div className="card-body">
              <h4 className="card-title">Manage Education Medium</h4>
