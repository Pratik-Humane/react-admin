...
const { getSubCastes } = MasterService1();
export default class SubCastes extends Component {
  ...
  DisabilitiesSubListData = async (offset, limit,search) => {
    this.setState({ isLoading: false });
      getSubCastes(offset, limit, search).then(async (res) => {
    ...
    return (
      <>
        ...
        <div className="col-lg-12 grid-margin stretch-card">
          <div className="card">
            <div className="card-body">
              <h4 className="card-title">Manage Sub Caste Category</h4>
                    ...
                    {this.state.result.map((row, index) => {
                      return (
                        <tr key={index}>
                          ...
                          <td>{row.main_cast && row.main_cast.category_name}</td>
