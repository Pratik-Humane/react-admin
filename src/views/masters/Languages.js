...
const { getLanguages } = MasterService1();
class Languages extends Component {
  state = { result: [], isLoading: false, offset: 0, limit: 10, count: 0, pages: [], selected: 0, };

  componentDidMount() {
    this.getLanguagesData(this.state.offset, this.state.limit, this.state.search);
  }

  // get Language list
  getLanguagesData = async (offset, limit, search) => {
    getLanguages(offset, limit, search).then(async (res) => {
  ...
  changePage = (offset) => {
    ...
    this.getLanguagesData(offset * this.state.limit, this.state.limit);
  };

  // search filter
  searchFilter = () => {
    this.getLanguagesData(0, 10, this.state.search);
  };

  // reset filter
  resetFilter = () => {
    this.setState({ ...this.state, search: null });
    this.getLanguagesData(this.state.selected, this.state.limit, null);
    ...
    return (
      <>
        ...
        <div className="col-lg-12 grid-margin stretch-card">
          <div className="card">
            <div className="card-body">
              <h4 className="card-title">Manage Languages</h4>
