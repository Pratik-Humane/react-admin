...
const { getExperience } = MasterService();
class Experience extends Component {
  state = { result: [], isLoading: false, offset: 0, limit: 10, count: 0, pages: [], selected: 0, };

  componentDidMount() {
    this.getExperienceData(this.state.offset, this.state.limit, this.state.search);
  }

  // get Boards list
  getExperienceData = async (offset, limit, search) => {
      getExperience(offset, limit, search).then(async (res) => {
  ...
  changePage = (offset) => {
    ...
    this.getExperienceData(offset * this.state.limit, this.state.limit);
  };

  // search filter
  searchFilter = () => {
    this.getExperienceData(0, 10, this.state.search);
  };

  // reset filter
  resetFilter = () => {
    this.setState({ ...this.state, search: null });
    this.getExperienceData(this.state.selected, this.state.limit, null);
    ...
    return (
      <>
        ...
        <div className="col-lg-12 grid-margin stretch-card">
          <div className="card">
            <div className="card-body">
              <h4 className="card-title">Manage Experiences</h4>
