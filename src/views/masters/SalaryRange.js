...
const { getSalaryRange } = MasterService();
class SalaryRange extends Component {
  state = { salary_range: [], isLoading: false, offset: 0, limit: 10, count: 0, pages: [], selected: 0, };

  componentDidMount() {
    this.getSalaryRangeListData(this.state.offset, this.state.limit, this.state.search);
  }

  // get SalaryRange list
  getSalaryRangeListData = async (offset, limit, search) => {
    this.setState({ isLoading: false });
    getSalaryRange(offset, limit, search).then(async (res) => {
      ...
      if (response.data.status === 200) {
        ...
        this.setState({ isLoading: true, salary_range: response.data.result, count: response.data.count, pages: pages, });
      } else {
  ...
  changePage = (offset) => {
    ...
    this.getSalaryRangeListData(offset * this.state.limit, this.state.limit);
  };

  // search filter
  searchFilter = () => {
    this.getSalaryRangeListData(0, 10, this.state.search);
  };

  // reset filter
  resetFilter = () => {
    this.setState({ ...this.state, search: null });
    this.getSalaryRangeListData(this.state.selected, this.state.limit, null);
    ...
    return (
      <>
        ...
        <div className="col-lg-12 grid-margin stretch-card">
          <div className="card">
            <div className="card-body">
              <h4 className="card-title">Manage Salary Range</h4>
              ...
              <div className="table-responsive">
                <table className="table table-bordered">
                  ...
                  <tbody>
                    ...
                    {this.state.salary_range.map((salary, index) => {
                      return (
                        <tr key={index}>
                          <td> {this.state.limit * this.state.selected + (index + 1)} </td>
                          <td>{salary.range}</td>
                          <td> {salary.status === "1" ? (<span className="text text-success">Active</span>) : (<span className="text text-danger">Inactive</span>)} </td>
                        </tr>
                      );
                    })}
                  </tbody>
