...
const { getMills } = MasterService1();
class Mills extends Component {
  state = { result: [], isLoading: false, offset: 0, limit: 10, count: 0, pages: [], selected: 0, };

  componentDidMount() {
    this.getMillsData(this.state.offset, this.state.limit, this.state.search);
  }

  // get Mills list
  getMillsData = async (offset, limit, search) => {
    getMills(offset, limit, search).then(async (res) => {
  ...
  changePage = (offset) => {
    ...
    this.getMillsData(offset * this.state.limit, this.state.limit);
  };

  // search filter
  searchFilter = () => {
    this.getMillsData(0, 10, this.state.search);
  };

  // reset filter
  resetFilter = () => {
    this.setState({ ...this.state, search: null });
    this.getMillsData(this.state.selected, this.state.limit, null);
    ...
    return (
      <>
        ...
        <div className="col-lg-12 grid-margin stretch-card">
          <div className="card">
            <div className="card-body">
              <h4 className="card-title">Manage Mills</h4>
